...
class Boton:
    def __init__(self, x, y, w, h, image_or_color=None, texto="", color_texto=(255, 255, 255)):
        ...
        if isinstance(image_or_color, pygame.Surface):
            ...
            # Crear una máscara de la imagen para detectar clics solo en las áreas visibles
            self.mask = pygame.mask.from_surface(self.image)
        else:
            # Si el argumento es un color, usarlo como fondo
            self.image = None
            self.color_fondo = image_or_color
            self.mask = None  # No se necesita máscara si es un color sólido
    ...
    def is_clicked(self):
        ...
        # Verificar si el mouse está dentro del rectángulo del botón
        if self.rect.collidepoint(pos):
            if self.image and self.mask:
                # Convertir la posición del mouse a la posición relativa de la imagen
                relative_pos = (pos[0] - self.rect.x, pos[1] - self.rect.y)
                # Verificar si el mouse está sobre una parte visible de la imagen usando la máscara
                if self.mask.get_at(relative_pos):
                    if pygame.mouse.get_pressed()[0]:  # Detectar si el botón izquierdo del mouse está presionado
                        return True
...
num_rondas_inicial = 0
background_seleccion = pygame.image.load('menu/menu2/background.jpeg')
def menu_seleccion():
    global num_rondas, n_jugadores, skins, current_screen, muted, puntajes, num_rondas_inicial
...
while ejecuta:
    ...
    if current_screen == 'menu':
        ...
        texto_ganar = font.render(f'The player {best_player[0]} has won', True, COLOR_BLANCO)
        texto_tecla =  font.render(f'Press any key', True, COLOR_BLANCO)
        texto_tecla2 = font.render(f'     to continue', True, COLOR_BLANCO)
